function scattering_transform_for_all_training_files(current_path, input_data_path)
    fid = fopen(input_data_path);
    disp('scattering transform for all training files:')
    tline = fgetl(fid);
	...
	disp(tline)
        ...
        save_foler = [current_path,'/preprocessing','/scat_coefficients_training/'];
